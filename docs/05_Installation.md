...
본 문서는 APMS.SR 프로젝트를 실행하고 개발하기 전에 필요한 호스트 개발 환경(Prerequisites)을 준비하는 Bootstrap 문서​입니다.

프로젝트 소스 코드를 생성하거나 초기화하지 않으며, 이미 존재하는 저장소를 실행할 수 있도록 Java, Node.js, Docker, Git 등의 필수 개발 도구를 설치하고 정상 동작 여부를 확인합니다.

프로젝트의 실제 실행, Docker Compose 기동, 환경변수 설정 및 Health Check는 다음 단계인 02_Quick_Start.md에서 다룹니다.
